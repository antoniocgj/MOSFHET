...
void _test_normal_generator_print_test(){
  for (size_t i = 0; i < 100000; i++){
    printf("%lx ", double2torus(generate_normal_random(0.001)));
  }
  exit(0);
}
...
void test_tlwe(){
  TLWE_Key key = tlwe_new_key(n, lwe_std_dev);
  Torus in1, in2;
  ...
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 54, in1, tlwe_phase(c_1, key), "TLWE decryption failed");
  TLWE c_2 = tlwe_new_sample(in2, key);
  TLWE c_3 = tlwe_new_noiseless_trivial_sample(0, n);
  tlwe_add(c_3, c_1, c_2);
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 54, in1+in2, tlwe_phase(c_3, key), "TLWE addition failed");
  tlwe_addto(c_3, c_1);
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 54, in1+in2+in1, tlwe_phase(c_3, key), "TLWE addto failed");
  tlwe_sub(c_3, c_1, c_2);
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 54, in1-in2, tlwe_phase(c_3, key), "TLWE sub failed");
  tlwe_subto(c_3, c_1);
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 54, in1-in2-in1, tlwe_phase(c_3, key), "TLWE subto failed");
  ...
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 58, in1, tlwe_phase(c_3, key), "10000 TLWE additions failed");
...
void test_trlwe(){
  TRLWE_Key key = trlwe_new_key(N, k, rlwe_std_dev);
  ...
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 44, poly_1->coeffs, poly_res->coeffs, N, "TRLWE decryption failed.");
  ...
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 44, poly_sum->coeffs, poly_res->coeffs, N, "TRLWE addition failed.");

  trlwe_sub(c_3, c_1, c_2);
  polynomial_sub_torus_polynomials(poly_sum, poly_1, poly_2);
  trlwe_phase(poly_res, c_3, key);
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 44, poly_sum->coeffs, poly_res->coeffs, N, "TRLWE subtraction failed.");
...
void test_compressed_trlwe(){
  ...
  polynomial_sub_torus_polynomials(poly_sum, poly_1, poly_2);
  trlwe_phase(poly_res, c_1, key);
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 44, poly_sum->coeffs, poly_res->coeffs, N, "Compressed TRLWE subto failed.");
...
void test_poly_DFT(){
  ...
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 30, poly->coeffs, poly_res->coeffs, N, "DFT transform failed.");
...
void test_poly_DFT_mul(){
  ...
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 40, poly_res_2->coeffs, poly_res_1->coeffs, N, "DFT multiplication failed.");

  polynomial_mul_addto_DFT(poly_dft_3, poly_dft_1, poly_dft_2);
  polynomial_DFT_to_torus(poly_res_1, poly_dft_3);
  polynomial_naive_mul_addto_torus(poly_res_2, poly_1, poly_2);
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 40, poly_res_2->coeffs, poly_res_1->coeffs, N, "DFT mul_addto failed.");
  free_polynomial(poly_1);
  free_polynomial(poly_2);
...
void test_poly_int128_mul(){
  ...
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 62, poly_4->coeffs, poly_3->coeffs, N, "Karatsuba u128 mul failed.");
...
void test_trgsw(){
  TRLWE_Key trlwe_key = trlwe_new_key(N, k, rlwe_std_dev);
  int test;
  generate_random_bytes(sizeof(int), (uint8_t *) &test);
  test &= (N - 1);
...
void test_trgsw_trlwe_mul(){
  ...
  generate_random_bytes(N*8, (uint8_t *) poly_1->coeffs);
  ...
  memset(poly_clear->coeffs, 0, N*8);
  memset(poly_2->coeffs, 0, N*8);
  poly_2->coeffs[test] = 1;
  polynomial_naive_mul_addto_torus(poly_clear, poly_1, poly_2);
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 44, poly_clear->coeffs, poly_res->coeffs, N, "External product failed.");
  free_trlwe_key(trlwe_key);
  free_trgsw_key(key);
  free_trgsw(c_1);
  free_trgsw(c_1_dft);
...
void _test_tlwe_ks_save_keys_and_exit(){
  TLWE_Key key_1 = tlwe_new_key(n, lwe_std_dev);
  TLWE_Key key_2 = tlwe_new_key(N, rlwe_std_dev);
...
void test_tlwe_ks(){
  ...
  // printf("in2: %lx, c2: %lx c_2_k1: %lx\n", in2, tlwe_phase(c_2, key_2), tlwe_phase(c_2_k1, key_1));
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 58, in2, tlwe_phase(c_2_k1, key_1), "TLWE ks_key_2_to_1 failed");
  ...
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 58, in1, tlwe_phase(c_1_k2, key_2), "TLWE ks_key_1_to_2 failed");
...
void test_tlwe_pack1_ks(){
  TLWE_Key key_1 = tlwe_new_key(n, lwe_std_dev);
  TRLWE_Key key_2 = trlwe_new_key(N, k, rlwe_std_dev);
  ...
  TEST_ASSERT_INT64_ARRAY_WITHIN_MESSAGE(1UL << 58, res->coeffs, out->coeffs, N, "TLWE(m) -> TRLWE(m*X^0) Key Switching failed");
...
void test_tlwe_pack1_ks_CDKS21(){
  TLWE_Key key_1 = tlwe_new_key(N, rlwe_std_dev);
  TRLWE_Key key_2 = trlwe_new_key(N, k, rlwe_std_dev);
...
void test_tlwe_pack_key_priv_ks(){
  ...
  TEST_ASSERT_INT64_ARRAY_WITHIN_MESSAGE(1UL << 58, res->coeffs, out->coeffs, N, "Private Key Switching TLWE(M) -> TRLWE(m*-s) failed");
...
void test_multivalue_bootstrap_CLOT21(){
  TLWE_Key key_tlwe = tlwe_new_key(n, lwe_std_dev);
  TLWE_Key key_tlwe_out = tlwe_new_key(N, rlwe_std_dev);
  TRLWE_Key key_trlwe = trlwe_new_key(N, k, rlwe_std_dev);
  ...
  for (size_t i = 0; i < 8; i++){
    TEST_ASSERT_INT64_WITHIN_MESSAGE(1UL << 58, lut[8 + i], tlwe_phase(out[i], key_tlwe_out), "Multi value bootstrap CLOT21 failed.");
  }
  fflush(stdout);
...
void test_circuit_bootstrap(){
  ...
  TLWE_Key key_2_extracted = tlwe_new_key(N, rlwe_std_dev);
  ...
  TEST_ASSERT_INT64_ARRAY_WITHIN_MESSAGE(1UL << 58, p[0]->coeffs, p[1]->coeffs, N, "Circuit Bootstrap LWE(1/4) -> TRGSW (1) failed");
  ...
  TEST_ASSERT_INT64_ARRAY_WITHIN_MESSAGE(1UL << 58, p[0]->coeffs, p[1]->coeffs, N, "Circuit Bootstrap LWE(0/4) -> TRGSW (0) failed");
...
void test_public_mux(){
  const int bit_len = sizeof(Torus)*8;
  TRLWE_Key key = trlwe_new_key(N, k, 0);
  ...
  TEST_ASSERT_INT64_ARRAY_WITHIN_MESSAGE(1UL << 56, p[1]->coeffs, p[2]->coeffs, N, "Public MUX s=1 failed");
  ...
  public_mux(out, p[0], p[1], selector, l, Bg_bit);
  trlwe_phase(p[2], out, key);
  TEST_ASSERT_INT64_ARRAY_WITHIN_MESSAGE(1UL << 56, p[0]->coeffs, p[2]->coeffs, N, "Public MUX s=0 failed");
...
void test_FDFB_KS21(){
  ...
  for (size_t i = 0; i < 8; i++){
    ...
    TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 58, in[i], tlwe_phase(c_out, key_tlwe_out), "FDFB [KS21] failed.");
  }
...
void test_FDFB_new(){
  ...
  for (size_t i = 0; i < 8; i++){
    ...
    TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 56, in[i], tlwe_phase(c_out, key_tlwe_out), "FDFB failed.");
  }
...
void test_FDFB_CLOT21(){
  ...
  for (size_t i = 0; i < 8; i++){
    ...
    TEST_ASSERT_INT64_WITHIN_MESSAGE(1ULL << (64 - precision - 1), in[i], tlwe_phase(c_out, key_tlwe_out), "FDFB [CLOT21] failed.");
  }
...
void test_trlwe_poly_mul(){
  ...
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 58, res_e->coeffs, res->coeffs, N, "TRLWE poly mul failed");
...
void test_trlwe_ks(){
  TRLWE_Key key_1 = trlwe_new_key(N, k, rlwe_std_dev);
  TRLWE_Key key_2 = trlwe_new_key(N, k, rlwe_std_dev);
  ...
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 58, in2->coeffs, c_2_k1_r->coeffs, N, "TRLWE ks_key_2_to_1 failed");
  ...
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 58, in1->coeffs, c_1_k2_r->coeffs, N, "TRLWE ks_key_1_to_2 failed");
...
void test_trlwe_packing_ks(){
  TLWE_Key key_tlwe = tlwe_new_key(N, rlwe_std_dev);
  TRLWE_Key key_trlwe = trlwe_new_key(N, k, rlwe_std_dev);
...
void test_blind_rotate(){
  ...
  Torus in;
  generate_random_bytes(8, (uint8_t *) &in);
  ...
  TEST_ASSERT_HEX64_ARRAY_WITHIN_MESSAGE(1UL << 58, poly_res_2->coeffs, poly_out->coeffs, N, "Blind Rotate failed.");
  free_trgsw(tmp);
  free_trgsw(s);
  free_trlwe(lut);
  free_polynomial(poly_out);
...
void test_functional_bootstrap_unfolded(){
  TLWE_Key key_tlwe = tlwe_new_key(632, lwe_std_dev); // n must be multiple of unfolding
  TLWE_Key key_tlwe_out = tlwe_new_key(N, rlwe_std_dev);
  TRLWE_Key key_trlwe = trlwe_new_key(N, k, rlwe_std_dev);
  trlwe_extract_tlwe_key(key_tlwe_out, key_trlwe);
  ...
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 58, in[1], tlwe_phase(c[4], key_tlwe_out), "Bootstrap failed. (cleartext LUT, unfolding = 2)");
  
  free_bootstrap_key(bk_key);
  bk_key = new_bootstrap_key(trgsw_key, key_tlwe, 4);
  functional_bootstrap(c[4], lut_c, c[5], bk_key, 4);

  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 58, in[1], tlwe_phase(c[4], key_tlwe_out), "Bootstrap failed. (cleartext LUT, unfolding = 4)");
  ...
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 58, in[1], tlwe_phase(c[4], key_tlwe_out), "Bootstrap failed. (cleartext LUT, unfolding = 8)");
...
void test_programmable_bootstrap(){
  ...
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 58, in[1], tlwe_phase(c[4], key_tlwe_out), "Bootstrap with cleartext LUT failed 0,0.");

  tlwe_sample(c[5], int2torus(0xA, 6), key_tlwe);
  programmable_bootstrap(c[4], lut_c, c[5], bk_key, 3, 3, 0);

  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 58, in[2], tlwe_phase(c[4], key_tlwe_out), "Bootstrap with cleartext LUT failed 3,0.");
...
void test_functional_bootstrap(){
  ...
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 58, in[1], tlwe_phase(c[4], key_tlwe_out), "Bootstrap with cleartext LUT failed.");
  ...
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 58, in[1], tlwe_phase(c[4], key_tlwe_out), "Bootstrap with encrypted LUT failed.");
  free_tlwe_key(key_tlwe);
  free_tlwe_key(key_tlwe_out);
  free_trlwe_key(key_trlwe);
  free_trgsw_key(trgsw_key);
...
void test_functional_bootstrap_trgsw(){
  ...
  Torus in[5];
  TLWE c[6];
  generate_random_bytes(8*4, (uint8_t *) in);
  for (size_t i = 0; i < 5; i++){
    c[i] = tlwe_new_sample(in[i], key_tlwe_out);
  }
  c[5] = tlwe_new_sample(double2torus(1./8), key_tlwe);
...
void test_functional_mv_bootstrap(){
  ...
  TEST_ASSERT_HEX64_WITHIN_MESSAGE(1UL << 58, double2torus(((double) lut_int[1])/8), tlwe_phase(c[4], key_tlwe_out), "Multi value Bootstrap with cleartext LUT failed.");
  free_tlwe_key(key_tlwe);
  free_tlwe_key(key_tlwe_out);
  free_trlwe_key(key_trlwe);
  free_trgsw_key(trgsw_key);
